...
class TaskInstance(Base):
    ...
    def run(
        ...
        try:
            ...
            os.environ['BODEGA_CLIENT_LOCAL_DAG_ID'] = self.dag_id
            os.environ['BODEGA_CLIENT_LOCAL_TASK_ID'] = self.task_id
			...
			os.environ.pop('BODEGA_CLIENT_LOCAL_DAG_ID', '')
            os.environ.pop('BODEGA_CLIENT_LOCAL_TASK_ID', '')
        # Recording SUCCESS
        self.end_date = datetime.now()
        self.set_duration()
        if not test_mode:
        ...
        if unfinished_tasks and none_depends_on_past:
            ...
            for ut in unfinished_tasks:
                ...
                deps_met = ut.are_dependencies_met(
                    ...
                    session=session)
                new_state = ut.current_state(session=session)
                if deps_met or new_state != old_state:
